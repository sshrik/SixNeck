...
int main() {
	...
	int ms[DIR_MAX][MAP_LENGTH][MAP_LENGTH] = { 0, };
	int es[DIR_MAX][MAP_LENGTH][MAP_LENGTH] = { 0, };
	mapping(map);
	msMapping(ms);
	...
	while (1) {
		...
		if (button == 32) {
			...
			else if(turn%4 ==3 || turn%4 == 0){
				map[y - 1][x / 2 - 1] = 2;
				turn++;
			}
			system("cls");
			search_state(ms, es, map, BLACK);
			mapping(map);
			msMapping(ms);
...
int get_state(int map[][MAP_LENGTH], int mine, vector start) {
	...
	if (mine == WHITE) {
		// Check _State
		if ((start.y > MAP_LENGTH - 7) && (dir.x == 0 && dir.y == 1)) {  //EAST 견제
			return NONE_STATE;
		}
		else if ((start.x > MAP_LENGTH - 7) && (dir.x == 1 && dir.y == 0)) { //SOUTH 견제
			return NONE_STATE;
		}
		else if ((start.y < 7 || start.x > MAP_LENGTH - 7) && (dir.x == -1 && dir.y == 1)) { //NORTH_EAST 견제
			return NONE_STATE;
		}
		else if ((start.y > MAP_LENGTH - 7 || start.x > MAP_LENGTH - 7) && (dir.x == 1 && dir.y == 1)) {   //SOUTH_EAST 견제
			return NONE_STATE;
		}
